...
let size_of_string s =
  try int_of_string s with Failure "int_of_string" -> err_invalid_size s
...
module PIC = struct
  ...
  let with_cache f sha1 =
    ...
    | Some x -> x
    | None   ->
      Log.debug "%a: cache miss!" SHA.output sha1;
      let x = f () in
  ...
  and unpack_kind = function
    | Raw x  -> x
    | Link d ->
      Log.debug "unpack: hop to %a" SHA.output d.source.sha1;
...
module IO (D: SHA.DIGEST) (I: Inflate.S) = struct
  ...
  let to_pic ~digest ~read ~offsets ~sha1s t =
    ...
    Log.debug "to_pic(%a) -> %a:%a"
      (Misc.output (Misc.pretty pp_kind)) t.kind
      SHA.output sha1
      (Misc.output PIC.pretty_kind) kind;
  ...
  let rec unpack_ref_delta ~lv ~version ~index ~read ba d =
    Log.debug "unpack-ref-delta[%d]: d.source=%a" lv SHA.output d.source;
